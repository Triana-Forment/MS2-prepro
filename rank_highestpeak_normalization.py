...
def rank_TIC_normalization(input_file, output_file):
    ...
    for spectrum in reader:
        ...
        max_peak = np.max(spectrum["intensity array"]) # Highest peak intensity
        ...
        for index in range(spec_len):
            normalized[index] = np.sum(sorted_int[:index+1])/max_peak
...
if __name__ == '__main__':
    ...
    rank_highestpeak_normalization(*sys.argv[1:])
